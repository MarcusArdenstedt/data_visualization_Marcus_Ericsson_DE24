    ...
    with tgb.part(class_name= "container card"):
        with tgb.layout("1 1 1"):
            with tgb.part() as column_fruit:
                ...
                tgb.slider(value="{slider_value}", min=1, max=50, step=1, continuous=True)
            ...
            with tgb.part() as column_calculator:
                ...
                tgb.input("{number1}", on_change=clear_results)  # on_change -> this function will run whne value is change
                
                
                tgb.text("Type in another number")
                tgb.input("{number2}", on_change=clear_results)
                
                tgb.text("You have type in {number1} and {number2}")
